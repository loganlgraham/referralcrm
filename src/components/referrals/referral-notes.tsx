...
export function ReferralNotes({
  ...
  const viewedNoteIdsRef = useRef<Set<string>>(new Set(initialNotes.map((note) => note.id)));

  const activityFeedKey = `/api/referrals/${referralId}/activities`;
  const notesEndpoint = `/api/referrals/${referralId}/notes`;
  ...
  const { data: fetchedNotes } = useSWR<ReferralNote[]>(
    notesEndpoint,
    async (url: string) => {
      ...
      return (await response.json()) as ReferralNote[];
    },
    { refreshInterval: 15000, revalidateOnFocus: true, fallbackData: initialNotes }
  );
  ...
  useEffect(() => {
    ...
    const newNotes = fetchedNotes.filter((note) => !viewedNoteIdsRef.current.has(note.id));
    if (newNotes.length === 0) return;

    if (showNotesDropdown) {
      newNotes.forEach((note) => viewedNoteIdsRef.current.add(note.id));
      setUnseenNotes(0);
    } else {
      setUnseenNotes((count) => count + newNotes.length);
    }
  }, [fetchedNotes, showNotesDropdown]);

  useEffect(() => {
    if (!showNotesDropdown) return;
    setUnseenNotes(0);
    notes.forEach((note) => viewedNoteIdsRef.current.add(note.id));
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      viewedNoteIdsRef.current.add(notePayload.id);
      setUnseenNotes(0);
      resetForm();
            ...
            <div className="rounded border border-slate-200 bg-slate-50 p-3 shadow-sm">
              <div className="flex items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{formatTimestamp(latestNote.createdAt)}</p>
                </div>
                <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                  Latest
                </span>
              </div>
              <p className="mt-2 whitespace-pre-line text-sm text-slate-700">{latestNote.content}</p>
              {(() => {
                const showVisibilityBadge =
                  viewerRole === 'admin' && (latestNote.hiddenFromAgent || latestNote.hiddenFromMc);
                const showEmailBadge =
                  Array.isArray(latestNote.emailedTargets) && latestNote.emailedTargets.length > 0;
                  ...
                  <div className="mt-2 flex flex-wrap gap-2 text-xs font-semibold uppercase tracking-wide">
                    {showVisibilityBadge && (
                      <span className="rounded-full bg-amber-100 px-2 py-0.5 text-amber-700">
                        {[
                          latestNote.hiddenFromAgent ? 'Hidden from agent' : null,
                          latestNote.hiddenFromMc ? 'Hidden from MC' : null
                        ]
                          .filter(Boolean)
                          .join(' • ')}
                      </span>
                    ...
                    {showEmailBadge && (
                      <span className="rounded-full bg-emerald-100 px-2 py-0.5 text-emerald-700">
                        {`Emailed: ${latestNote.emailedTargets
                          ?.map((target) => {
                            if (target === 'agent') {
                              return 'Agent';
                            }
              ...
              <div className="space-y-2">
                ...
                >
                  <span className="flex items-center gap-2">
                    <span>Show details ({remainingNotes.length})</span>
                    {unseenNotes > 0 && !showNotesDropdown && (
