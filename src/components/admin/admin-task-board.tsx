...
export function AdminTaskBoard() {
  ...
  const boardUrl = `/api/admin/tasks/board?groupBy=${groupBy}&status=${statusFilter}`;
  const { data: groups = [], mutate } = useSWR<BoardGroup[]>(boardUrl, fetcher);
  ...
  const handleManualSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const referralId = manualReferralId.trim();
    if (!title || !referralId) {
      toast.error('Add a task name and referral ID before saving.');
      return;
    }
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      {showManualForm && (
        <form onSubmit={handleManualSubmit} className="space-y-2 rounded-lg border border-slate-200 bg-slate-50 p-4">
          <input
            type="text"
