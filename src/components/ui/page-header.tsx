...
export function PageHeader({
  ...
  return (
    <header className={cn('relative flex flex-col gap-4 border-b border-border pb-5 pl-5 sm:flex-row sm:items-end sm:justify-between', className)}>
      <span aria-hidden className="absolute bottom-5 left-0 top-0 w-[3px] rounded-full bg-primary-600" />
      ...
      <div className="min-w-0 space-y-1.5">
        ...
        <h1 className="truncate font-display text-2xl font-semibold tracking-[-0.035em] text-foreground sm:text-[1.85rem]">
          {title}
        </h1>
        {description && (
