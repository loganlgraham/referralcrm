  ...
  const sectionsHtml = args.sections
    .map((section) => {
      const body = section.rows.length
        ? `<table role="presentation" style="border-collapse:collapse;margin:6px 0 14px;width:100%;"><tbody>${section.rows
            .map(
              (row) =>
                `<tr><td style="padding:4px 14px 4px 0;font-family:${EMAIL_FONT_STACK};font-size:14px;color:${EMAIL_COLORS.foregroundMuted};">${escapeHtml(
                  row.label
                )}</td><td style="padding:4px 0;font-family:${EMAIL_FONT_STACK};font-size:14px;font-weight:600;color:${EMAIL_COLORS.foreground};">${escapeHtml(
                  row.value
                )}</td></tr>`
            )
            .join('')}</tbody></table>`
        : `<p style="margin:6px 0 14px;font-family:${EMAIL_FONT_STACK};font-size:14px;color:${EMAIL_COLORS.foregroundSubtle};font-style:italic;">${escapeHtml(
            section.emptyMessage ?? 'No data available.'
          )}</p>`;
      return `<section><h3 style="margin:18px 0 4px;font-family:${EMAIL_FONT_STACK};font-size:16px;font-weight:600;color:${EMAIL_COLORS.foreground};">${escapeHtml(section.title)}</h3>${body}</section>`;
    })
    .join('');

  return renderEmailHtml({
    preheader: `${args.reportName} · ${args.windowLabel}`,
    heading: args.reportName,
    bodyHtml: `<p style="margin:0 0 16px 0;font-family:${EMAIL_FONT_STACK};font-size:13px;line-height:20px;color:${EMAIL_COLORS.foregroundMuted};">Timeframe: ${escapeHtml(args.windowLabel)} &nbsp;&middot;&nbsp; Network: ${escapeHtml(args.network)}</p>${sectionsHtml}`,
  });
}
